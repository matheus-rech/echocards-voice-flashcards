  ...
  updateCard: (updatedCard: Card): void => {
    try {
      ...
      errorLogger.logError(
        ...
        { function: 'updateCard', cardId: updatedCard.id }
      );
  ...
  createDeck: (name: string): Deck => {
    try {
      ...
      errorLogger.logError(
        ...
        ErrorCategory.STORAGE,
        ErrorSeverity.HIGH,
        { function: 'createDeck', deckName: name }
      );
      throw error;
    }
  },
  ...
  createCard: (deckId: string, question: string, answer: string, explanation?: string): Card => {
    try {
      ...
      errorLogger.logError(
        ...
        ErrorCategory.STORAGE,
        ErrorSeverity.HIGH,
        { function: 'createCard', deckId, question: question.substring(0, 50) }
      );
      throw error;
    }
  },
  importDeckFromCsv: (deckName: string, csvContent: string): Deck => {
    try {
      ...
      lines.forEach((line, index) => {
        try {
          ...
          errorLogger.logError(
            ...
            ErrorCategory.STORAGE,
            ErrorSeverity.LOW,
            { function: 'importDeckFromCsv', line, index }
          );
        }
      });
      ...
      errorLogger.logError(
        ...
        ErrorCategory.STORAGE,
        ErrorSeverity.HIGH,
        { function: 'importDeckFromCsv', deckName }
      );
      throw error;
    }
  },
