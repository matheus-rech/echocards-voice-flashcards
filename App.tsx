...
const App: React.FC = () => {
  ...
  const handleRateCard = useCallback((ratingStr: 'AGAIN' | 'HARD' | 'GOOD' | 'EASY') => {
    if (sessionState === SessionState.AWAITING_RATING && currentCard) {
      ...
      if (remainingCards.length > 0) {
        ...
        toast.error('Failed to save card rating. Please try again.');
        setSessionState(SessionState.AWAITING_RATING);
      }
    }
  }, [sessionState, currentCard, reviewQueue, sessionProgressCount, activeGoal, studyProgressData, playTts, toast]);
  ...
  const handleCreateDeck = useCallback(async (deckName: string) => {
    try {
      ...
      toast.error('Failed to create deck. Storage may be full.');
      const text = `Sorry, I couldn't create the "${deckName}" deck. Please try again.`;
      setStatusText(text);
      playTts(text);
    }
  }, [playTts, toast]);
  ...
  const handleDeleteDeck = useCallback(async (deckName: string) => {
    ...
    if (deckToDelete) {
        const success = storageService.deleteDeck(deckToDelete.id);
        if (success) {
          ...
          const text = `Sorry, I couldn't delete the "${deckName}" deck.`;
          setStatusText(text);
          playTts(text);
        }
    } else {
  ...
  const handleCreateCard = useCallback(async (deckName: string, question: string, answer: string, explanation?: string) => {
    ...
    if (targetDeck) {
      try {
        ...
        toast.error('Failed to create card. Storage may be full.');
        const text = `Sorry, I couldn't create that card. Please try again.`;
        setStatusText(text);
        playTts(text);
      }
    } else {
  ...
  const handleUpdateCardContent = useCallback(async (updates: { newQuestion?: string; newAnswer?: string; newExplanation?: string }) => {
    ...
    try {
      ...
      const text = "Sorry, I couldn't update the card. Please try again.";
      setStatusText(text);
      playTts(text);
    }
  }, [cardToEdit, sessionState, playTts, toast]);
  ...
  const handleImportDeck = useCallback(async (deckName: string, csvContent: string) => {
    try {
      ...
      setStatusText(text);
      playTts(text);
      toast.success(`Deck "${deckName}" imported successfully!`);
    } catch (error) {
      toast.error('Failed to import deck. Check file format or storage space.');
      setSessionState(SessionState.AWAITING_COMMAND);
      const text = `Sorry, I couldn't import the "${deckName}" deck. Please check the file format.`;
      setStatusText(text);
      playTts(text);
    }
  }, [playTts, toast]);
  ...
  const handleShowBackupView = useCallback(() => {
    setSessionState(SessionState.SHOWING_BACKUP);
    setStatusText('Backup and restore your flashcard data.');
    playTts('You can export your data to JSON, CSV, or Anki format, or import a backup file.');
  }, [playTts]);
  ...
  const renderContent = () => {
    switch(sessionState) {
      ...
      case SessionState.SHOWING_BACKUP:
        return <BackupView onClose={handleGoBack} />;
      default:
